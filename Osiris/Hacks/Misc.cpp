...
void Misc::watermark() noexcept
{
    ...
    // Display cazdev text
    ImGui::Text("cazdev | %d fps | %d ms", frameRate != 0.0f ? static_cast<int>(1 / frameRate) : 0, GameData::getNetOutgoingLatency());
    ImGui::End();
}
...
void Misc::drawOffscreenEnemies(ImDrawList* drawList) noexcept
{
    ...
    for (auto& player : GameData::players()) {
        ...
        const auto color = Helpers::calculateColor(miscConfig.offscreenEnemies);
...
void Misc::drawGUI(bool contentOnly) noexcept
{
    ...
    if (ImGui::BeginPopup("")) {
        ...
        if (miscConfig.offscreenEnemies.healthBar.type == HealthBar::Solid) {
            ImGui::SameLine();
            ImGuiCustom::colorPicker("", static_cast<Color4&>(miscConfig.offscreenEnemies.healthBar));
        }
        ImGui::EndPopup();
    }
    ImGui::PopID();
